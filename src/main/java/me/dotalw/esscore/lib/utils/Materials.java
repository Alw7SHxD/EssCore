...
public enum Materials {
    ...
    public static Materials requestMaterials(String name, byte data) {
        if (cachedSearch.containsKey(String.format("%s,%s", name.toUpperCase(), data)))
            return cachedSearch.get(String.format("%s,%s", name.toUpperCase(), data));

        for (Materials material : Materials.values())
            for (String test : material.aliases)
                if (name.toUpperCase().equals(test) && ((byte) material.data) == data) {
                    cachedSearch.put(String.format("%s,%s", test, data), material);
                    return material;
                }
        return null;
    }

    public boolean isSameMaterial(ItemStack item) {
        if (isNewVersion()) return item.getType() == this.parseMaterial();
        if (item.getType() == this.parseMaterial() && (int) item.getData().getData() == this.data)
            return true;

        Materials material = fromMaterial(item.getType());
        return isDamageable(material) && this.parseMaterial() == item.getType();
    }

    public Materials fromMaterial(Material material) {
        try {
            return Materials.valueOf(material.toString());
        } catch (IllegalArgumentException e) {
            for (Materials mat : Materials.values()) {
                for (String test : mat.aliases) {
    ...
    public static Materials fromString(String key) {
        ...
        try {
            material = Materials.valueOf(key);
            return material;
        } catch (IllegalArgumentException e) {
            String[] split = key.split(":");
            if (split.length == 1) {
                material = requestMaterials(key, (byte) 0);
            } else {
                material = requestMaterials(split[0], (byte) Integer.parseInt(split[1]));
            }
            return material;
        }
    ...
    public Material parseMaterial() {
        ...
        return Material.matchMaterial(aliases[0]);
    }
